...
query=input("query:")
if ("or" in query and "and" not in query):             #We have four selection,one has "or" but not "and".
        query=set(query.split(" "))
        query.remove("or")
        print ("Performing OR search for:"+str(query))
        query=list(query)                              #deal with the input string, delete the repeat word, and set it to list.
        orlist=[]
        for i,quote in enumerate(data_list):
                found_at = quote.find(query[0])        #find the quote in the data_list(), which has query[0].
                if(found_at >= 0):
                        orlist.append(i)
                found_at = quote.find(query[1])        #find the quote in the data_list(), which has query[1].              
                if(found_at >= 0):
                        orlist.append(i)
                found_at1 = quote.find(query[0])
                found_at2 = quote.find(query[1])
                if(found_at1 >= 0 and found_at2>=0):   #find the quote in the data_list(), which has query[0] and query[1].
                        orlist.append(i)
        print (orlist)                                 #put the list element number into orlist.
        orlist=list(set(orlist))
        orlist.sort()
        for i in orlist:
                print("Found at ", i, data_list[i])
elif ("and" in query and "or" in query):               #one has both "and" and "or" string.
        query=set(query.split(" "))
        query.remove("and")
        query.remove("or")
        ...
        query=list(query)
        andlist1=[]
        for i,quote in enumerate(data_list):           #find the quote in the data_list(), which has query[0] and query[1].
                found_at1 = quote.find(query[0])
                found_at2 = quote.find(query[1])
                if(found_at1 >= 0 and found_at2>=0):
                        andlist1.append(i)
        andlist1=list(set(andlist1))
        andlist1.sort()
        for i in andlist1:
                print("Found at ", i, data_list[i])
elif ("and" in query and "or" not in query):           #one has "and" but not "or".
        query=set(query.split(" "))
        query.remove("and")
        print ("Performing AND search for:"+str(query))
        query=list(query)
        ...
        for i,quote in enumerate(data_list):           #find the quote in the data_list(), which has query[0] and query[1].
                ...
                if(found_at1 >= 0 and found_at2 >=0):
                        andlist2.append(i)
        andlist2=list(set(andlist2))
        andlist2.sort()
        for i in andlist2:
                print("Found at ", i, data_list[i])
elif ("or" not in query and "and" not in query):       # one neight has "and" nor "or".
        query=set(query.split(" "))
        print ("Performing AND search for:"+str(query))
        query=list(query)
        notlist = []
